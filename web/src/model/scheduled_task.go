...
type ScheduledTask struct {
	...
	ScheduleType   string      `gorm:"type:varchar(32);index:idx_scheduled_task_status_schedule_execution"` // Schedule type: one-time, daily, weekly, monthly
	ExecutionTime  time.Time   `gorm:"index:idx_scheduled_task_status_schedule_execution"`                   // Next execution time for one-time tasks
	...
	Status         string      `gorm:"type:varchar(32);index:idx_scheduled_task_status_schedule_execution"` // Task status: enabled, disabled
}
