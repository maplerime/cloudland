...
func (v *IpGroupAPI) List(c *gin.Context) {
	...
	dicID := strings.TrimSpace(c.DefaultQuery("dic_id", "")) // Retrieve dictionary_id from query params
	logger.Debugf("List ipGroups with offset %s, limit %s, query %s, dic_id %s", offsetStr, limitStr, queryStr, dicID)
	if err != nil {
	...
	if dicID != "" {
		logger.Debugf("Filtering ipGroups by dictionary_id: %s", dicID)
		var dictionary *model.Dictionary
		dictionary, err := dictionaryAdmin.GetDictionaryByUUID(ctx, dicID)
		if err != nil {
			logger.Errorf("Invalid query dic_id: %s, %+v", dicID, err)
			ErrorResponse(c, http.StatusBadRequest, "Invalid query ipGroups by dic_id UUID: "+dicID, err)
			return
		}

		logger.Debugf("The dictionary with dic_id: %+v\n", dictionary)
		logger.Debugf("The dic_id in dictionary is: %d", dictionary.ID)
