...
func (a *AlarmAPI) DeleteCPURule(c *gin.Context) {
	...
	patterns := []string{
		...
		fmt.Sprintf("%s/cpu-special-%s.yml", common.RulesSpecial, groupUUID),
		fmt.Sprintf("%s/cpu-general-%s.yml", common.RulesEnabled, groupUUID),
		fmt.Sprintf("%s/cpu-special-%s.yml", common.RulesEnabled, groupUUID),
	}
	...
	for _, pattern := range patterns {
		...
		for _, path := range matches {
			if err := common.RemoveFile(path); err != nil {
...
func (a *AlarmAPI) DisableRules(c *gin.Context) {
	...
	pattern := filepath.Join(common.RulesEnabled, fmt.Sprintf("*%s*", groupUUID))
	matches, _ := filepath.Glob(pattern)
...
func generateBWRuleContent(rules []common.BWRule, groupName string, groupUUID string, vms []string, isSpecial bool, targetDevice string) (string, error) {
	...
	// 将 inst-xxx:eth0 拆分为 inst-xxx 和 eth0，便于后续处理
	for _, vm := range vms {
...
func (a *AlarmAPI) DeleteBWRules(c *gin.Context) {
	...
	if err == nil {
		for _, link := range vmLinks {
			vmName, err := routes.GetInstanceUUIDByDomain(c.Request.Context(), link.VMUUID)
			if err != nil {
				log.Printf("convert VM UUIDfailed uuid=%s error=%v", link.VMUUID, err)
				continue
			}
			excludeVMs = append(excludeVMs, vmName)
		}
	}
	generalPath, specialPath := common.RulePaths(common.RuleTypeBW, groupUUID)
	if len(excludeVMs) > 0 {
		if content, err := os.ReadFile(generalPath); err == nil {
			newContent := removeExcludedVMs(string(content), excludeVMs)
			_ = common.AtomicWrite(generalPath, newContent)
	...
	patterns := []string{
		...
		fmt.Sprintf("%s/bw-special-%s.yml", common.RulesSpecial, groupUUID),
		fmt.Sprintf("%s/bw-general-%s.yml", common.RulesEnabled, groupUUID),
		fmt.Sprintf("%s/bw-special-%s.yml", common.RulesEnabled, groupUUID),
	}

	for _, pattern := range patterns {
		matches, _ := filepath.Glob(pattern)
		log.Printf("[clean] mode: %s, match: %v", pattern, matches)
		for _, path := range matches {
			if err := common.RemoveFile(path); err != nil {
				log.Printf("[clean] delete %s failed: %v", path, err)
			}
		}
	}
	if err := common.ReloadPrometheus(); err != nil {
