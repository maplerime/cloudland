...
func (a *AlarmAPI) CreateCPURule(c *gin.Context) {
	...
	if len(req.LinkedVMs) > 0 {
		for _, vmUUID := range req.LinkedVMs {
			...
			if _, err := a.n9eOperator.CreateVMRuleLink(c.Request.Context(), link); err != nil {
...
// ============================================================
// Disk Rule Handlers
// Uses N9EDiskRule model (same family as N9ECPURule / N9EMemoryRule / N9EBandwidthRule).
// Per-VM thresholds live in VictoriaMetrics anchor labels only — not in DB.
// ============================================================
...
func (a *AlarmAPI) CreateDiskRule(c *gin.Context) {
	...
	// Duplicate check
	if _, err := a.n9eOperator.GetDiskRuleByUUID(c.Request.Context(), req.RuleID); err == nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "disk rule already exists with rule_id: " + req.RuleID})
		return
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to query existing disk rule: " + err.Error()})
		return
	}
	...
	// Step 2: Create N9E Disk Rule
	diskRule := &model.N9EDiskRule{
		RuleID:            req.RuleID,
		BusinessGroupUUID: businessGroup.UUID,
		Name:              req.Name,
		Owner:             req.Owner,
	...
	if err := a.n9eOperator.CreateDiskRule(c.Request.Context(), diskRule); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to create disk rule: " + err.Error()})
		return
	}
	...
	if a.n9eClient != nil && a.templateRenderer != nil {
		...
		if templateErr != nil {
			...
			// Step 4: Get/create N9E business group in N9E API and create alert rule
			n9eGroupID, bgErr := a.n9eClient.GetOrCreateBusinessGroup(c.Request.Context(), regionName)
			if bgErr != nil {
				...
				if n9eGroupID > 0 && datasourceID > 0 {
					n9eRule := routes.N9EAlertRule{
						...
						Disabled:         0,
						DatasourceIDs:    []int64{datasourceID},
						PromForDuration:  durationMin * 60,
						PromEvalInterval: 60,
						NotifyRepeatStep: 60,
						RuleConfig: routes.N9ERuleConfig{
							Queries: []routes.N9EQuery{
					...
					if ruleErr != nil {
						...
						if err := a.n9eOperator.UpdateDiskRuleN9EID(c.Request.Context(), req.RuleID, n9eAlertRuleID); err != nil {
...
func (a *AlarmAPI) GetDiskRules(c *gin.Context) {
	...
	buildResponse := func(rule model.N9EDiskRule) gin.H {
		vmLinks, _ := a.n9eOperator.GetVMRuleLinks(c.Request.Context(), rule.RuleID)
		linkedVMs := make([]string, 0)
		for _, lk := range vmLinks {
			linkedVMs = append(linkedVMs, lk.VMUUID)
		}
		var regionID, level string
		if bg, err := a.n9eOperator.GetBusinessGroupByUUID(c.Request.Context(), rule.BusinessGroupUUID); err == nil && bg != nil {
	...
	if ruleID != "" {
		rule, err := a.n9eOperator.GetDiskRuleByUUID(c.Request.Context(), ruleID)
		if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		c.JSON(http.StatusOK, gin.H{
			"data": []gin.H{buildResponse(*rule)},
			"meta": gin.H{"total": 1, "current_page": 1, "per_page": 1, "total_pages": 1},
		})
		return
	}

	params := routes.ListN9ERulesParams{Page: page, PageSize: pageSize}
	rules, total, err := a.n9eOperator.ListDiskRules(c.Request.Context(), params)
	if err != nil {
	...
	responseData := make([]gin.H, 0, len(rules))
	for _, r := range rules {
		responseData = append(responseData, buildResponse(r))
	}
	c.JSON(http.StatusOK, gin.H{
		"data": responseData,
		"meta": gin.H{
...
func (a *AlarmAPI) DeleteDiskRule(c *gin.Context) {
	...
	// Step 1: Get disk rule
	diskRule, err := a.n9eOperator.GetDiskRuleByUUID(c.Request.Context(), ruleID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		c.JSON(http.StatusNotFound, gin.H{"error": "disk rule not found", "code": "NOT_FOUND", "uuid": ruleID})
		return
	} else if err != nil {
	...
	if err != nil {
		logger.Warningf("Failed to get business group %s: %v", diskRule.BusinessGroupUUID, err)
	}
	...
	if diskRule.N9EAlertRuleID > 0 && a.n9eClient != nil && businessGroup != nil {
		...
		if bgErr != nil {
			...
			if delErr := a.n9eClient.DeleteAlertRule(c.Request.Context(), n9eBGID, diskRule.N9EAlertRuleID); delErr != nil {
				logger.Warningf("Failed to delete N9E alert rule ID %d: %v", diskRule.N9EAlertRuleID, delErr)
			} else {
				logger.Infof("Deleted N9E alert rule ID %d from BG %d", diskRule.N9EAlertRuleID, n9eBGID)
			}
		}
	}

	// Step 7: Delete disk rule from local database
	if err := a.n9eOperator.DeleteDiskRule(c.Request.Context(), ruleID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "DB operation failed: " + err.Error(), "code": "DB_DELETE_FAILED"})
		return
	}
			...
			if a.n9eClient != nil {
				...
				if bgErr != nil {
					...
					if delErr := a.n9eClient.DeleteBusinessGroupByID(c.Request.Context(), n9eBGID); delErr != nil {
						...
						logger.Infof("Auto-deleted N9E business group '%s' (ID=%d)", businessGroup.Name, n9eBGID)
					}
				}
			}
		}
...
func (a *AlarmAPI) LinkVMsToRule(c *gin.Context) {
	...
	case "disk":
		diskRule, err := a.n9eOperator.GetDiskRuleByUUID(c.Request.Context(), req.RuleUUID)
		if err != nil {
			c.JSON(http.StatusNotFound, gin.H{"error": "Disk rule not found"})
			return
		}
		owner = diskRule.Owner
		businessGroupUUID = diskRule.BusinessGroupUUID
		anchorType = "disk"
	default:
	...
	// Upsert VM rule links in database (all types use N9EVMRuleLink)
	createdCount := 0
	updatedCount := 0
	for i, vm := range req.VMs {
		iface := vm.Interface
		...
		created, err := a.n9eOperator.CreateVMRuleLink(c.Request.Context(), link)
		if err != nil {
			logger.Warningf("Failed to upsert VM rule link: %v", err)
		} else if created {
			createdCount++
		} else {
			updatedCount++
	...
	logger.Infof("Successfully linked %d VMs to rule %s (anchorType=%s, threshold=%g, created=%d, updated=%d)",
		len(req.VMs), req.RuleUUID, anchorType, req.Threshold, createdCount, updatedCount)
	c.JSON(http.StatusOK, gin.H{
		"status":        "success",
...
func (a *AlarmAPI) GetRuleLinks(c *gin.Context) {
	...
	resolveAnchorTypes := func(rt string) []string {
		...
		default:
			// 未指定则查全部 5 种
			return []string{"cpu", "mem", "bw_in", "bw_out", "disk"}
		}
	}
...
func (a *AlarmAPI) GetDBLinks(c *gin.Context) {
	...
	for rUUID, links := range linkMap {
		...
		case "disk":
			diskRule, err := a.n9eOperator.GetDiskRuleByUUID(c.Request.Context(), rUUID)
			if err == nil {
				ruleName = diskRule.Name
				ruleOwner = diskRule.Owner
				bg, _ := a.n9eOperator.GetBusinessGroupByUUID(c.Request.Context(), diskRule.BusinessGroupUUID)
				if bg != nil {
					businessGroupName = bg.Name
				}
			}
...
func (a *AlarmAPI) UnlinkVMsFromRule(c *gin.Context) {
	...
	// Delete VM rule links from database (all types use N9EVMRuleLink)
	_ = isDiskUnlink // no longer needed
