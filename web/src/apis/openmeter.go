...
func (o *OpenMeterAPI) processMetricsBySubject(subject, instanceID string, events []OpenMeterEvent, queryStart, queryEnd int64) (*MetricProcessingResult, error) {
	...
	case "wds_volume_size_bytes":
		processedData, summary, err := o.processVolumeStorageMetrics(events, queryStart, queryEnd)
		if err != nil {
			return nil, err
		}
		result.ProcessedData = processedData
...
// processVolumeStorageMetrics processes volume storage size metrics
// Rules: Same as CPU metrics but for volume storage size
func (o *OpenMeterAPI) processVolumeStorageMetrics(events []OpenMeterEvent, queryStart, queryEnd int64) (map[string]interface{}, map[string]interface{}, error) {
	return o.processResourceMetrics(events, "volume_storage", "bytes", queryStart, queryEnd, func(event OpenMeterEvent) interface{} {
		return o.extractVolumeStorageFromEvent(event)
	})
}
...
// extractVolumeStorageFromEvent extracts volume storage bytes from metric event
func (o *OpenMeterAPI) extractVolumeStorageFromEvent(event OpenMeterEvent) interface{} {
	return o.extractMemoryFromEvent(event) // Same extraction logic as memory
}
