...
func (a *IpGroupAdmin) GetIpGroupByUUID(ctx context.Context, uuID string) (ipGroup *model.IpGroup, err error) {
	...
	where := memberShip.GetWhere()
	ipGroup = &model.IpGroup{}
	err = db.Where(where).Where("uuid = ?", uuID).Preload("Subnets").Preload("FloatingIPs", "id ASC").Preload("FloatingIPs.Subnet").Preload("FloatingIPs.Subnet.Group").Preload("FloatingIPs.Subnet.Group.DictionaryType").Preload("FloatingIPs.Group").Preload("FloatingIPs.Group.DictionaryType").Preload("DictionaryType").Take(ipGroup).Error
	if err != nil {
		logger.Errorf("Failed to query ipGroup, %v", err)
		return
	}
...
func (a *IpGroupAdmin) List(ctx context.Context, offset, limit int64, order, query string) (total int64, ipGroups []*model.IpGroup, err error) {
	...
	if err = db.Preload("Subnets").Preload("DictionaryType").Preload("FloatingIPs", "id ASC").Preload("FloatingIPs.Subnet").Preload("FloatingIPs.Subnet.Group").Preload("FloatingIPs.Subnet.Group.DictionaryType").Preload("FloatingIPs.Group").Preload("FloatingIPs.Group.DictionaryType").Where(where).Where(query).Find(&ipGroups).Error; err != nil {
