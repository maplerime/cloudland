...
func (a *SecgroupAdmin) AllowInstanceLoginPort(ctx context.Context, port int32, iface *model.Interface) (err error) {
	...
	for _, sg := range iface.SecurityGroups {
		_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "ingress", "tcp", port, port, sg, false)
		if err != nil {
...
func (a *SecgroupAdmin) Create(ctx context.Context, name string, isDefault bool, router *model.Router) (secgroup *model.SecurityGroup, err error) {
	...
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "egress", "tcp", 1, 65535, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "egress", "udp", 1, 65535, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "ingress", "tcp", 22, 22, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "ingress", "tcp", 3389, 3389, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "ingress", "udp", 68, 68, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "egress", "icmp", -1, -1, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
	_, err = secruleAdmin.Create(ctx, "", "0.0.0.0/0", "ingress", "icmp", -1, -1, secgroup, false)
	if err != nil {
		logger.Error("Failed to create security rule", err)
		return
	}
