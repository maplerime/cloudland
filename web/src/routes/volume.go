...
func (a *VolumeAdmin) Update(ctx context.Context, id int64, name string, instID int64) (volume *model.Volume, err error) {
	...
	updates := map[string]interface{}{"status": volume.Status}
	if volume.Name != "" {
...
func (v *VolumeView) List(c *macaron.Context, store session.Store) {
	...
	SetPaginationData(c, "volumes", total, limit, offset, listConfig,
		...
		[]string{"ID", "UUID", "Path", "Name", "Size", "IopsLimit", "BpsLimit", "Status", "Bootable", "AttachedAs", "Hyper", "IsCopyClone", "Owner", "Action"})
