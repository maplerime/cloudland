...
func (a *ScheduledTaskAdmin) Create(ctx context.Context, name, taskType, resourceType string,
	...
	logger.Debugf("[Admin] Creating scheduled task - function entry: name=%s, taskType=%s, resourceType=%s, operation=%s, scheduleType=%s, cronExpression=%s, resourceID=%d, retentionCount=%d, executionTime=%s",
	...
	logger.Debugf("[Admin] Scheduled task created successfully: id=%d - function exit", task.ID)
	return
}
...
func (a *ScheduledTaskAdmin) Update(ctx context.Context, id int64, name, status, scheduleType, cronExpression string, retentionCount int, executionTime time.Time) (task *model.ScheduledTask, err error) {
	logger.Debugf("[Admin] Updating scheduled task - function entry: id=%d", id)
	...
	logger.Debugf("[Admin] Scheduled task %d updated successfully - function exit", id)
	return
}
...
func (a *ScheduledTaskAdmin) Delete(ctx context.Context, id int64) (err error) {
	logger.Debugf("[Admin] Deleting scheduled task - function entry: id=%d", id)
	...
	logger.Debugf("[Admin] Scheduled task %d deleted successfully - function exit", id)
	return
}
