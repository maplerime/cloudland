...
ip netns exec $router iptables -C FORWARD -d $ext_ip -m conntrack --ctstate NEW -j ACCEPT
[ $? -ne 0 ] && ip netns exec $router iptables -A FORWARD -d $ext_ip -m conntrack --ctstate NEW -j ACCEPT
