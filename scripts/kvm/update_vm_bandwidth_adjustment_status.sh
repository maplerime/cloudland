...
STATUS_METRIC_LINE="vm_bandwidth_adjustment_status{domain=\"$DOMAIN\", rule_id=\"$PROMETHEUS_RULE_ID\", type=\"$TYPE\", target_device=\"$TARGET_DEVICE\"} $STATUS"
...
if [[ "$STATUS" == "1" ]]; then
    ...
    TIMESTAMP=$(date +%s%3N)
    TIMESTAMP_METRIC_LINE="vm_bandwidth_limit_start_timestamp{domain=\"$DOMAIN\", rule_id=\"$PROMETHEUS_RULE_ID\", type=\"$TYPE\", target_device=\"$TARGET_DEVICE\"} $TIMESTAMP"
fi
