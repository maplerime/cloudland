...
if [ "$os_code" = "windows" ]; then
    ...
    if [ -n "$primary_ip" ]; then
        read -d'\n' -r ip netmask  < <(ipcalc -nb $primary_ip | awk '/Address/ {print $2} /Netmask/ {print $2}')
        net_json=$(jq --arg ip "$ip" --arg netmask "$netmask" --arg gateway "$gateway" '.networks |= map(if .id == "network0" then (.ip_address = $ip | .netmask = $netmask | .routes[0].gateway = $gateway) else . end)' <<< "$net_json")
    fi
