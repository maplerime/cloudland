...
if [ "$ret_code" != "0" ]; then 
    ...
    echo "|:-COMMAND-:| $(basename $0) '$backup_ID' 'error' ' ' 'failed to create snapshot: $message'"
    exit -1
fi
...
echo "|:-COMMAND-:| $(basename $0) '$backup_ID' '$state' 'wds_vhost://$wdsPoolID/$snapshot_id' 'success'"
