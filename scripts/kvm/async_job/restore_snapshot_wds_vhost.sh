...
if [ "$volume_pool_id" == "$snapshot_pool_id" ]; then
    ...
    restore_ret=$(wds_curl PUT "api/v2/sync/block/volumes/$volume_wds_uuid/recovery" -d "{\"snap_id\": \"$snapshot_wds_uuid\"}")
    ...
    if [ "$ret_code" != "0" ]; then
        ...
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' '$state' '$vol_path' 'failed to restore volume: $message'"
        exit -1
    fi
else
    ...
    clone_ret=$(wds_curl POST "api/v2/sync/block/volumes/$snapshot_wds_uuid/copy_clone" -d "{\"name\": \"$new_vol_name\", \"phy_pool_id\": \"$volume_pool_id\"}")
    ...
    if [ "$ret_code" != "0" ]; then
        ...
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'failed_to_restore' '$vol_path' 'failed to clone volume: $message'"
        exit -1
    fi
    ...
    if [ "$state" != "available" ]; then
        ...
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'failed_to_restore' '$vol_path' 'failed to clone volume: task state is $st'"
        exit -1
    fi
    ...
    if [ "$ret_code" != "0" ]; then
        ...
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'failed_to_restore' '$vol_path' 'failed to get volume by name: $message'"
        exit -1
    fi
    ...
    if [ -z "$new_volume_wds_uuid" ] || [ "$new_volume_wds_uuid" == "null" ]; then
        ...
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'failed_to_restore' '$vol_path' 'Could not find cloned volume by name'"
        exit -1
    fi
    ...
    if [ $instance_id -gt 0 ]; then
        ...
        if [ "$ret_code" != "0" ]; then
            echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'failed_to_restore' '$vol_path' 'failed to create wds vhost for boot volume, $vhost_ret, $uss_ret!'"
            exit -1
        fi
        echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' 'attached' '$vol_path' 'success'"
        exit 0
    fi
...
echo "|:-COMMAND-:| create_volume_wds_vhost '$origin_vol_ID' '$state' '$vol_path' 'success'"